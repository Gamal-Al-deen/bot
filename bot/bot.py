...
def handle_start_command(chat_id, user_id, first_name="مستخدم", username="لا يوجد"):
    ...
    try:
        ...
        log_error(f"✅ [START] Welcome message sent to user {user_id} (Admin: {is_admin}, New: {is_new})")
    
    except Exception as e:
        log_error(f"❌ [START] Error: {type(e).__name__}: {str(e)}")
        import traceback
        log_error(f"📋 [START] Full traceback:\n{traceback.format_exc()}")
...
def handle_my_account(chat_id, user_id, first_name="مستخدم", username="لا يوجد"):
    ...
    try:
        ...
        log_error(f"✅ [MY_ACCOUNT] User {user_id} found - Balance: {balance}")
        ...
        send_message(chat_id, text, reply_markup)
        log_error(f"✅ [MY_ACCOUNT] Account info sent to user {user_id}")
    
    except Exception as e:
        log_error(f"❌ [MY_ACCOUNT] Error: {type(e).__name__}: {str(e)}")
        import traceback
        log_error(f"📋 [MY_ACCOUNT] Full traceback:\n{traceback.format_exc()}")
        send_message(chat_id, f"❌ حدث خطأ أثناء جلب معلومات الحساب.\n\n📋 الخطأ: {str(e)}")
